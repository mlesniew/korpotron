...
- ✓ Logged-in user can select template, optionally pick options, enter text, generate result, copy to clipboard — all
...
paste a prompt into a general-purpose LLM chat tool from a text file.
...
- Clipboard copy is available without manually selecting result text; a pre-selected text box is an acceptable fallback
  if the one-click clipboard action is unavailable
- The original-message input is only shown when the selected template has the is-response flag set (see FR-010,
  nice-to-have)
...
choices, then submits the composite for text generation to produce a rewritten version of the user's input.

Option groups are structured so that only one option per group can be active at a time. This guarantees that the
assembled prompt contains no conflicting instructions — the composition rule is enforced by the option group structure,
not by the user at generation time. The user selects at most one option per group, and the app merges the selected
instruction texts into the base prompt before submitting it for text generation.
...
- **No prompt editing during normal use** — users select a template and options; they do not edit the raw prompt
  mid-workflow.
- **is-response / reply-context input deferred to v2** — the template is-response flag and its associated
  original-message input field are captured as FR-010 (nice-to-have) and are out of MVP scope.

## Open Questions

1. **`target_scale.qps` and `target_scale.data_volume`** — inferred as `low` / `small` based on single-digit user count.
   Confirm or correct before tech-stack selection if the anticipated traffic or data profile is different.
2. **Which text-generation service will be used** — not captured in this PRD by design; resolve during tech-stack
   selection. Owner: user.
3. **US-01 acceptance criteria vs FR-012** — the original acceptance criteria stated one-click copy; the Socratic review
   of FR-012 revised this to allow a pre-selected text box as a fallback. US-01 has been updated accordingly. Confirm
   the revised wording is acceptable before implementation.
